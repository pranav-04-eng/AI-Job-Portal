"""The voice interview as a LangGraph state machine.
...
This service uses MemorySaver (in-memory checkpoints). For production, swap in
the Postgres checkpointer so paused interviews survive restarts.
"""
import json
...
def await_answer(state: InterviewState) -> dict:
    ...
    new_history = state["history"] + [{"q": state["current_question"], "a": answer}]
